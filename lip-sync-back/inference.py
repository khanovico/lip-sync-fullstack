...
def start_inference(face, audio_s, outfile="results/result_voice.mp4", static_set = False, fps=24, fps_default=True):
    
    do_load("checkpoints/wav2lip.pth")
    ...
    if not audio_s.endswith('.wav'):
        ...
        audio_s = 'temp/temp.wav'

    wav = audio.load_wav(audio_s, 16000)
    mel = audio.melspectrogram(wav)
    print(mel.shape)
